...
export default function ArcherDashboard() {
  ...
  // Envoi du commentaire indépendamment de l'upload
    ...
    <div className="flex h-screen bg-gray-50 flex-col">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <p className="text-xs text-gray-500 border-l border-gray-200 pl-3">Bonjour, {user?.name}</p>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="flex items-center justify-between mt-3">
              <div className="flex gap-4 text-xs text-gray-500">
                ...
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-blue-500 inline-block" /> Entraînement</span>
              </div>
              {lastRefresh && <span className="text-[11px] text-gray-400">Mis à jour à {format(lastRefresh, 'HH:mm')}</span>}
            </div>
          </div>
        )}
        ...
        {tab === 'upload' && (
          <div className="flex-1 overflow-y-auto p-6">
            <div className="max-w-lg mx-auto space-y-4">
              ...
              {/* Bloc commentaire — indépendant de l'upload */}
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-3">
                <div>
                  <h2 className="font-bold text-gray-900 mb-0.5">💬 Message au coach</h2>
                  <p className="text-sm text-gray-500">Informez votre coach d'une contrainte ou d'une remarque pour cette semaine.</p>
                </div>
                <textarea
                  value={comment}
                ...
                <button
                  ...
                  className="w-full py-2 rounded-xl bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition disabled:opacity-40 disabled:cursor-not-allowed"
                >
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Uploadez une capture d'écran de votre emploi du temps. L'IA l'analysera automatiquement.</p>
                </div>
                <label className="block">
                  <div className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition ${
                    uploading ? 'border-blue-300 bg-blue-50 cursor-not-allowed' : 'border-gray-300 hover:border-blue-400 hover:bg-blue-50/30'
                  }`}>
                    <div className="text-3xl mb-2">{uploading ? '⏳' : '📷'}</div>
                    <p className="text-sm font-medium text-gray-700">{uploading ? 'Analyse en cours…' : 'Cliquez pour choisir une image'}</p>
